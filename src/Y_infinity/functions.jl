...
function K_1_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    return -Diagonal(
        SVector((ϵ - im) * J_P_1_dξ(ξ, λ, κ, ϵ, Λ), (ϵ + im) * J_P_2_dξ(ξ, λ, κ, ϵ, Λ)),
    ) / (1 + ϵ^2)
end

function K_2_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    return Diagonal(
        SVector((ϵ - im) * J_E_1_dξ(ξ, λ, κ, ϵ, Λ), (ϵ + im) * J_E_2_dξ(ξ, λ, κ, ϵ, Λ)),
    ) / (1 + ϵ^2)
end
...
function J_P_1_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    (; d) = Λ
    c = _c(κ, ϵ, Λ)
    return B_W_1(λ, κ, ϵ, Λ) *
           (
               P_1_dξ(ξ, λ, κ, ϵ, Λ) * ξ^-1 +
               2c * P_1(ξ, λ, κ, ϵ, Λ) +
               (d - 1) * P_1(ξ, λ, κ, ϵ, Λ) * ξ^-2
           ) *
           exp(c * ξ^2) *
           ξ^d
end

function J_P_2_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    (; d) = Λ
    c = _c(κ, ϵ, Λ)
    return B_W_2(λ, κ, ϵ, Λ) *
           (
               P_2_dξ(ξ, λ, κ, ϵ, Λ) * ξ^-1 +
               2conj(c) * P_2(ξ, λ, κ, ϵ, Λ) +
               (d - 1) * P_2(ξ, λ, κ, ϵ, Λ) * ξ^-2
           ) *
           exp(conj(c) * ξ^2) *
           ξ^d
end

function J_E_1_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    (; d) = Λ
    c = _c(κ, ϵ, Λ)
    return B_W_1(λ, κ, ϵ, Λ) *
           (
               E_1_dξ(ξ, λ, κ, ϵ, Λ) * ξ^-1 +
               2c * E_1(ξ, λ, κ, ϵ, Λ) +
               (d - 1) * E_1(ξ, λ, κ, ϵ, Λ) * ξ^-2
           ) *
           exp(c * ξ^2) *
           ξ^d
end

function J_E_2_dξ(ξ, λ, κ, ϵ, Λ::CGLParams)
    (; d) = Λ
    c = _c(κ, ϵ, Λ)
    return B_W_2(λ, κ, ϵ, Λ) *
           (
               E_2_dξ(ξ, λ, κ, ϵ, Λ) * ξ^-1 +
               2conj(c) * E_2(ξ, λ, κ, ϵ, Λ) +
               (d - 1) * E_2(ξ, λ, κ, ϵ, Λ) * ξ^-2
           ) *
           exp(conj(c) * ξ^2) *
           ξ^d
end
