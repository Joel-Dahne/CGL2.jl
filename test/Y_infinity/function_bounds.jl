@testset "Y_infinity_function_bounds" begin
    ...
    function exp_E_1_dξ(ξ, λ, κ, ϵ, Λ)
        a, b, c = CGL2._abc(κ, ϵ, Λ)
        return exp(c * ξ^2) * (2c * ξ * E_1(ξ, λ, κ, ϵ, Λ) + E_1_dξ(ξ, λ, κ, ϵ, Λ))
    end
    # Compute derivative of exp(conj(c) * ξ^2) * E_2(ξ)
    function exp_E_2_dξ(ξ, λ, κ, ϵ, Λ)
        a, b, c = CGL2._abc(κ, ϵ, Λ)
        return exp(conj(c) * ξ^2) *
               (2conj(c) * ξ * E_2(ξ, λ, κ, ϵ, Λ) + E_2_dξ(ξ, λ, κ, ϵ, Λ))
    end

    function exp_P_1_dξ(ξ, λ, κ, ϵ, Λ)
        a, b, c = CGL2._abc(κ, ϵ, Λ)
        return exp(c * ξ^2) * (2c * ξ * P_1(ξ, λ, κ, ϵ, Λ) + P_1_dξ(ξ, λ, κ, ϵ, Λ))
    end
    # Compute derivative of exp(conj(c) * ξ^2) * P_2(ξ)
    function exp_P_2_dξ(ξ, λ, κ, ϵ, Λ)
        a, b, c = CGL2._abc(κ, ϵ, Λ)
        return exp(conj(c) * ξ^2) *
               (2conj(c) * ξ * P_2(ξ, λ, κ, ϵ, Λ) + P_2_dξ(ξ, λ, κ, ϵ, Λ))
    end
    ...
    for ξ in [1, 1.01, 1.1, 2, 4, 8, 16, 32, 64] .* ξ₁
        ...
        @test abs(E_1_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.E_1_dξ * exp(-real(c) * ξ^2) * ξ^(1 / σ - d - real(λ) / κ + 1)
        @test abs(E_2_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.E_2_dξ * exp(-real(c) * ξ^2) * ξ^(1 / σ - d - real(λ) / κ + 1)
        @test abs(E_1_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.95C_Y.E_1_dξ * exp(-real(c) * ξ^2) * ξ^(1 / σ - d - real(λ) / κ + 1)
        @test abs(E_2_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.85C_Y.E_2_dξ * exp(-real(c) * ξ^2) * ξ^(1 / σ - d - real(λ) / κ + 1)

        @test abs(exp_E_1_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.exp_E_1_dξ * ξ^(1 / σ - d - real(λ) / κ - 1)
        @test abs(exp_E_2_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.exp_E_2_dξ * ξ^(1 / σ - d - real(λ) / κ - 1)
        @test abs(exp_E_1_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.95C_Y.exp_E_1_dξ * ξ^(1 / σ - d - real(λ) / κ - 1)
        @test abs(exp_E_2_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.85C_Y.exp_E_2_dξ * ξ^(1 / σ - d - real(λ) / κ - 1)
        ...
        @test abs(P_1_dξ(ξ, λ, κ, ϵ, Λ)) <= C_Y.P_1_dξ * ξ^(-1 / σ + real(λ) / κ - 1)
        @test abs(P_2_dξ(ξ, λ, κ, ϵ, Λ)) <= C_Y.P_2_dξ * ξ^(-1 / σ + real(λ) / κ - 1)
        @test abs(P_1_dξ(ξ, λ, κ, ϵ, Λ)) >= 0.95C_Y.P_1_dξ * ξ^(-1 / σ + real(λ) / κ - 1)
        @test abs(P_2_dξ(ξ, λ, κ, ϵ, Λ)) >= 0.85C_Y.P_2_dξ * ξ^(-1 / σ + real(λ) / κ - 1)

        @test abs(exp_P_1_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.exp_P_1_dξ * exp(real(c) * ξ^2) * ξ^(-1 / σ + real(λ) / κ + 1)
        @test abs(exp_P_2_dξ(ξ, λ, κ, ϵ, Λ)) <=
              C_Y.exp_P_2_dξ * exp(real(c) * ξ^2) * ξ^(-1 / σ + real(λ) / κ + 1)
        @test abs(exp_P_1_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.95C_Y.exp_P_1_dξ * exp(real(c) * ξ^2) * ξ^(-1 / σ + real(λ) / κ + 1)
        @test abs(exp_P_2_dξ(ξ, λ, κ, ϵ, Λ)) >=
              0.9C_Y.exp_P_2_dξ * exp(real(c) * ξ^2) * ξ^(-1 / σ + real(λ) / κ + 1)
        ...
        @test abs(J_E_1_dξ(ξ, λ, κ, ϵ, Λ)) <= C_Y.J_E_1_dξ * ξ^(1 / σ - real(λ) / κ - 2)
        @test abs(J_E_1_dξ(ξ, λ, κ, ϵ, Λ)) >= 0.95C_Y.J_E_1_dξ * ξ^(1 / σ - real(λ) / κ - 2)

        @test abs(J_E_2_dξ(ξ, λ, κ, ϵ, Λ)) <= C_Y.J_E_2_dξ * ξ^(1 / σ - real(λ) / κ - 2)
        @test abs(J_E_2_dξ(ξ, λ, κ, ϵ, Λ)) >= 0.85C_Y.J_E_2_dξ * ξ^(1 / σ - real(λ) / κ - 2)
        ...
        Q_hat, Q_hat_dξ = CGL2.Q_hat_infinity(γ₁, γ₂, κ, ϵ, ξ, Λ)
        H = K_2(ξ, λ, κ, ϵ, Λ) * CGL2.I_N(Q_hat, Λ)
        H_dξ =
            K_2_dξ(ξ, λ, κ, ϵ, Λ) * CGL2.I_N(Q_hat, Λ) +
            K_2(ξ, λ, κ, ϵ, Λ) * CGL2.I_N_dξ(Q_hat, Q_hat_dξ, Λ)

        @test abs(H[1, 1]) <= C_Y.H_11 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[1, 2]) <= C_Y.H_12 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[2, 1]) <= C_Y.H_21 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[2, 2]) <= C_Y.H_22 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[1, 1]) >= 0.8C_Y.H_11 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[1, 2]) >= 0.8C_Y.H_12 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[2, 1]) >= 0.7C_Y.H_21 * ξ^(1 / σ - real(λ) / κ - 3)
        @test abs(H[2, 2]) >= 0.7C_Y.H_22 * ξ^(1 / σ - real(λ) / κ - 3)

        @test abs(H_dξ[1, 1]) <= C_Y.H_11_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[1, 2]) <= C_Y.H_12_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[2, 1]) <= C_Y.H_21_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[2, 2]) <= C_Y.H_22_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        # IMPROVE: These bounds are pretty bad
        @test abs(H_dξ[1, 1]) >= 0.3C_Y.H_11_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[1, 2]) >= 0.5C_Y.H_12_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[2, 1]) >= 0.2C_Y.H_21_dξ * ξ^(1 / σ - real(λ) / κ - 4)
        @test abs(H_dξ[2, 2]) >= 0.3C_Y.H_22_dξ * ξ^(1 / σ - real(λ) / κ - 4)
    end
